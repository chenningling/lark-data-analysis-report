...
- `dashboard_blocks`：图表名称、来源表名、图表类型、维度、指标和筛选条件。
- `report_outline`：报告章节，以及每个章节要引用的 Base 表/仪表盘产物。
...
Base 仪表盘图表是可交互的数据源事实，但最终飞书文档应包含静态图表图片，方便阅读。优先从同一批结果表生成本地图表图片，再用 `lark-cli docs +media-insert` 插入文档。
