...
- `references/calculation-planning.md`：把用户目标、计算步骤、Base 表、图表、仪表盘组件和报告块串成可追溯链路。
...
- `analysis_sections`：每个正文分析章节的业务问题、方法、步骤 ID、证据表、图表 ID、报告块 ID、短答案、置信度、限制和行动建议。
- `chart_registry`：每个图表 ID 对应的来源表、仪表盘组件、图片文件和报告章节。
- `dashboard_blocks`：由 `analysis_sections` 推导出的图表名称、来源表名、图表类型、维度、指标和筛选条件。
- `doc_blocks`：飞书文档分块发布顺序，保证章节文字与对应图片相邻。

规划时使用 `Qxx -> Sxx -> 90+_图表数据_* -> CHxx -> SECxx -> RBxx` 的 ID 链。每个正文章节都应能回溯到计算步骤、Base 表、仪表盘组件和文档图片。
...
为保证可重复写回，优先使用 `scripts/prepare_analysis_package.py` 生成标准本地产物包：Base 表 CSV、图表规格、报告 Markdown 块和发布清单。默认不把全量原始明细写入 Base；Base 只沉淀数据字典、清洗记录、关键中间结果、图表数据和结论索引。只有用户明确要求“逐行审计”“把源明细也放进 Base”时，才使用 `--include-source-rows`。

```bash
python3 scripts/prepare_analysis_package.py \
  --input input.xlsx \
  --goal "给出下个季度规划建议" \
  --output ./runs/<task-name> \
  --title "数据分析报告"
```
...
- 每张 Base 表对应的 CSV 文件。
- 报告 Markdown 文件或按章节拆分的 Markdown 块文件。
- 使用 ECharts 渲染的 PNG 图表图片文件。
...
- `97_图表注册表`：图表 ID、章节 ID、来源表、仪表盘组件、图片文件和报告块 ID。
...
正式报告固定使用 ECharts + Puppeteer 渲染 PNG。首次使用或换环境后，先在技能目录安装并检查 Node 依赖：
...
```bash
node scripts/render_echarts_png.mjs --specs ./runs/<task-name>/chart_specs.json
```

图表规格见 `references/chart-images.md`。图片文件名与图表/组件名称保持一致，并在 `97_图表注册表` 记录 `渲染方式 = ECharts PNG`。

### 6. 撰写飞书云文档报告

使用 `lark-cli docs +create` 创建报告。报告较长时，先创建简洁初稿，再用 `docs +update --mode append` 分段追加。通过发布脚本发布时，优先使用 manifest 的 `doc.blocks`，按章节 Markdown 和图片交错发布；旧格式 `doc.markdown` + `doc.images` 只适合图片统一追加到文末的简单报告。
