...
使用 `lark-cli base +base-create` 创建新的 Base，名称建议跟随分析任务，例如 `数据分析过程记录仓库 - <主题> - YYYYMMDD`。
...
使用 `lark-cli base +table-create`、`+field-create` 和 `+record-batch-create` 写入。记录批量写入时每批最多 200 行。

### 4. 创建 Base 仪表盘

使用 `lark-cli base +dashboard-create` 为本次分析创建一个仪表盘。再使用 `lark-cli base +dashboard-block-create` 基于最终洞察表添加组件。
...
使用 `lark-cli docs +create` 创建报告。报告较长时，先创建简洁初稿，再用 `docs +update --mode append` 分段追加。
